...
## Start the Turtlebot3 Bringup

First, power on and connect to the Turtlebot3, and start the drivers:
```
$ ssh ubuntu@32.80.100.108  # change IP to correspond to each TB3
$ ros2 launch turtlebot3_bringup robot.launch.py [remote robot]
```
In each new terminal used on the host PC, export the corresponding Turtlebot3 environemnt variables:

```
$ export ROS_DOMAIN_ID=8  # change for each Turtlebot
$ export TURTLEBOT3_MODEL=burger
$ export RMW_IMPLEMENTATION=rmw_fastrtps_cpp
```

## Launch Nav2 with custom params and filters
